...
let startedGettingDatasetList = false;
let startedGettingDatasetListId = null;
let startedGettingDatasetListEntity = null;
...
class DatasetList extends React.Component {
    ...
    async ionViewWillEnter() {
        ...
        document.title = "Datasets" + window.globalVars.pageTitle;

        // Desktop route causes rendering twice, if same id leave while the first reqiest is processed
        if ( (startedGettingDatasetList) || (this.state.list !== null && startedGettingDatasetListId === this.props.match.params.id && startedGettingDatasetListEntity === this.props.match.params.entity) ) {
            return;
        }

        startedGettingDatasetList = true;
        startedGettingDatasetListId = this.props.match.params.id;
        startedGettingDatasetListEntity = this.props.match.params.entity;

        setTimeout(function() {
            startedGettingDatasetList = false;
        }.bind(this), 1500);
    ...
    async getDatasetSlice() {
        ...
        if (this.state.tabTitle === "Datasets") {
        ...
        Server.api({
            method: "get",
            url: "/dataset/" + this.props.entity + "/" + this.props.match.params.id + "/" + this.state.page + "/" + (this.state.keywords ? this.state.keywords : "null"),
            then: function(res) {
                ...
                if (res.data) {
                    ...
                    if (res.data.length === 0) {
                        if (document.getElementById('infinite-scroll-dataset-list'))
                            document.getElementById('infinite-scroll-dataset-list').disabled = true
                        this.setState({
                            loading: false,
                            list: existing
                        })
                    ...
                    if (document.getElementById('infinite-scroll-dataset-list'))
                        document.getElementById('infinite-scroll-dataset-list').complete();
    ...
    render() {
        return (
            <IonPage data-page="dataset-list">
                <IonHeader>
                    <IonToolbar>
                ...
                <IonContent>
                    ...
                    { (this.state.list && this.state.list.length > 0) &&
                        <IonList>
                            { this.state.list.map(item => { return (<DatasetItem key={item.id} own={this.state.own} {...this.props} dataset={item} /> ) }) }
                        </IonList>
                    }
                    { (this.state.list && this.state.list.length === 0) &&
                    ...
                    { (this.state.list && this.state.list.length > 0) &&
                        <IonInfiniteScroll
                            threshold="50px"
                            id="infinite-scroll-dataset-list"
